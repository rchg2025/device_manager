...
async function getFromAddress() {
  ...
  if (!fromValue) {
    return `"Hệ thống quản lý thiết bị phòng thực hành, xưởng" <${userSetting?.value || "noreply@nsg.edu.vn"}>`
  }
  
  if (fromValue.includes('@') && !fromValue.includes('<')) {
    return `"Hệ thống quản lý thiết bị phòng thực hành, xưởng" <${fromValue}>`
  }
  
  if (!fromValue.includes('@') && !fromValue.includes('<')) {
    return `"${fromValue}" <${userSetting?.value || "noreply@nsg.edu.vn"}>`
  }
...
<body>
  <div class="container">
    ...
    <div class="footer">
      Email tự động từ Hệ thống quản lý thiết bị phòng thực hành, xưởng.<br>
      Trân trọng,<br>
...
export async function sendBorrowRequestEmailToAdmins(
  ...
  await transporter.sendMail({
    from,
    to: adminEmails.join(", "),
    subject: `[Hệ thống quản lý thiết bị phòng thực hành, xưởng] Yêu cầu mượn thiết bị mới từ ${memberName}`,
...
export async function sendReturnRequestEmailToAdmins(adminEmails: string[], memberName: string, equipmentName: string) {
  ...
  await transporter.sendMail({
    from,
    to: adminEmails.join(", "),
    subject: `[Hệ thống quản lý thiết bị phòng thực hành, xưởng] Yêu cầu trả thiết bị từ ${memberName}`,
...
export async function sendOtpEmail(email: string, userName: string, otp: string) {
  ...
  const content = `
    ...
    <p>Bạn vừa yêu cầu đặt lại mật khẩu cho tài khoản trên Hệ thống quản lý thiết bị phòng thực hành, xưởng.</p>
  ...
  if (transporter) {
    await transporter.sendMail({
      ...
      to: email,
      subject: `[Hệ thống quản lý thiết bị phòng thực hành, xưởng] Mã xác thực (OTP) đặt lại mật khẩu`,
      html: emailHtml
    })
  }
}

export async function sendStatusUpdateEmailToMember(
  memberEmail: string, 
  item: { name: string, image?: string, quantity: number, borrowDate: string | Date, returnDate: string | Date }, 
  ...
  let content = `
    <p>Xin chào,</p>
    <p>Yêu cầu mượn/trả thiết bị <strong>${item.name}</strong> của bạn đã được cập nhật trạng thái thành: <strong style="color: ${color};">${statusText}</strong>.</p>
    ...
    <div style="background: #f8fafc; padding: 16px; border-radius: 6px; border: 1px solid #e2e8f0; margin-bottom: 24px; font-size: 14px; color: #475569;">
      <p style="margin: 0 0 8px 0;"><strong>Người xử lý:</strong> ${processorName}</p>
      <p style="margin: 0;"><strong>Thời gian xử lý:</strong> ${new Date(processingTime).toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' })}</p>
    </div>
  `
  ...
  await transporter.sendMail({
    from,
    to: memberEmail,
    subject: `[Hệ thống quản lý thiết bị phòng thực hành, xưởng] Trạng thái thiết bị: ${statusText}`,
...
export async function sendBorrowRequestEmailToMember(
  ...
  await transporter.sendMail({
    from,
    to: memberEmail,
    subject: `[Hệ thống quản lý thiết bị phòng thực hành, xưởng] Xác nhận gửi yêu cầu mượn thiết bị`,
...
export async function sendReturnRequestEmailToMember(memberEmail: string, memberName: string, equipmentName: string) {
  ...
  await transporter.sendMail({
    from,
    to: memberEmail,
    subject: `[Hệ thống quản lý thiết bị phòng thực hành, xưởng] Xác nhận gửi yêu cầu trả thiết bị`,
...
export async function sendStatusToggleEmailToMember(memberEmail: string, memberName: string, isActive: boolean) {
  ...
  const content = `
    <p>Xin chào <span class="highlight">${memberName}</span>,</p>
    <p>Tài khoản của bạn trên Hệ thống quản lý thiết bị phòng thực hành, xưởng vừa được cập nhật trạng thái thành: <strong style="color: ${color};">${statusText}</strong>.</p>
  ...
  await transporter.sendMail({
    from,
    to: memberEmail,
    subject: `[Hệ thống quản lý thiết bị phòng thực hành, xưởng] Trạng thái tài khoản: ${statusText}`,
