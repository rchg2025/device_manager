...
export async function GET(request: Request) {
  try {
    ...
    const excelData = maintenances.map((mt, index) => ({
      "STT": index + 1,
      "Tên thiết bị": mt.classroomEq?.name || "Đã xoá",
      "Vị trí": `${mt.classroomEq?.room?.name || "N/A"} - ${mt.classroomEq?.area?.name || "N/A"}`,
      "Quản lý": mt.classroomEq?.room?.manager?.name || "",
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "LichSuBaoTriPhongHoc")

    const wscols = [
      { wch: 5 }, { wch: 30 }, { wch: 25 }, { wch: 20 }, 
    ...
    return new NextResponse(buf, {
      status: 200,
      headers: {
        "Content-Disposition": `attachment; filename="LichSuBaoTri_PhongHoc_${new Date().toISOString().slice(0,10)}.xlsx"`,
